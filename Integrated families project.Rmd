---
title: "Integrated families project.rmd"
...
write_csv(dtSample, "Sample.csv.gz")
```

```{r}
#Extract RelatedPatient table
(dtrelatedpatient <- dbReadTable(con, "RelatedPatient") %>% as_tibble)
write_csv(dtrelatedpatient, "RelatedPatient.csv.gz")
```

```{r}
#Extract PatientCase table. This table was difficult to get out of the HistoTrac database. Portions of the table had to be extracted separately, and then re-joined in R.
ls_colnm <- c(dbGetQuery(con, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='PatientCase'")$COLUMN_NAME)
rownumber <- dbGetQuery(con, "SELECT count(*) FROM PatientCase")
dtPC1 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[1:3], collapse = ', '), " FROM PatientCase")) 
dtPC2 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[4], collapse = ', '), " FROM PatientCase"))
dtPC3 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[5], collapse = ', '), " FROM PatientCase"))
dtPC4 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[6:10], collapse = ', '), " FROM PatientCase"))
dtPC5 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[11:16], collapse = ', '), " FROM PatientCase"))
(dtpatientcase <- bind_cols(dtPC1, dtPC2, dtPC3, dtPC4, dtPC5) %>% as_tibble)
write_csv(dtpatientcase, 'PatientCase.csv.gz')
```

```{r}
#Extract Patient table. The Patient table was difficult to get out. Portions of the table had to be extracted separately, and then re-joined in R. 
ls_colnm <- c(dbGetQuery(con, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='Patient'")$COLUMN_NAME)
rownumber <- dbGetQuery(con, "SELECT count(*) FROM Patient")
dtPS1 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[1:175], collapse = ', '), " FROM Patient"))
dtPS2 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[176:200], collapse = ', '), " FROM Patient"))
dtPS3 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[201:232], collapse = ', '), " FROM Patient"))
dtPS4 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[233:248], collapse = ', '), " FROM Patient"))
dtPS5 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[249], collapse = ', '), " FROM Patient"))
dtPS6 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[250], collapse = ', '), " FROM Patient"))
dtPS7 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[251:262], collapse = ', '), " FROM Patient"))
(dtP <- bind_cols(dtPS1, dtPS2, dtPS3, dtPS4, dtPS5, dtPS6, dtPS7) %>% as_tibble)
write_csv(dtP, 'Patient.csv.gz')
```

```{r}
#Extract Test table. The Test table had similar problems, and so had to be taken out in pieces. This is one of the largest tables, and may take a while to extract.
ls_colnm <- c(dbGetQuery(con, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='Test'")$COLUMN_NAME)
rownumber <- dbGetQuery(con, "SELECT count(*) FROM Test")
dtTS1 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[1:50], collapse = ', '), " FROM Test"))
dtTS2 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[51:100], collapse = ', '), " FROM Test"))
dtTS3 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[101:120], collapse = ', '), " FROM Test"))
dtTS4 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[121:143], collapse = ', '), " FROM Test"))
dtTS5 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[144:150], collapse = ', '), " FROM Test"))
dtTS6 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[151:200], collapse = ', '), " FROM Test"))
dtTS7 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[201:250], collapse = ', '), " FROM Test"))
dtTS8 <- dbGetQuery(con, paste("SELECT TOP ", rownumber, paste0(ls_colnm[251:276], collapse = ', '), " FROM Test"))
(dtTest <- bind_cols(dtTS1, dtTS2, dtTS3, dtTS4, dtTS5, dtTS6, dtTS7, dtTS8) %>% as_tibble)
write_csv(dtTest, 'Test.csv.gz')
```
...
```{r}
#This links patients with the PatientCase table, then the RelatedPatient table. This will add new rows whenever a patient has a donor.  
(linked_cases <- 
   (simple_patients %>%
...
(linked_families <-
 ...
 #This simplifies the table.
  %>% select(PatientId:RelationshipTypeCd)
